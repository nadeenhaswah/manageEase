                        ...
                        <div class="row  border border-gray-200 rounded-lg">
                            ...
                            <div class="switch col-md-6 mt-1 p-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    $cards_status_sql = "SELECT cd.status, cd.due_date 
                                    FROM cards c
                                    ...
                                    if (mysqli_num_rows($cards_status_result) > 0) {
                                        ...
                                        while ($card_status = mysqli_fetch_assoc($cards_status_result)) {
                                            ...
                                            if ($card_status['due_date'] && $card_status['due_date'] < $current_date && $card_status['status'] != 'Done') {
                                                $has_overdue_cards = true;
                                            }
                                        }
                                    }
                                    ...
                                    if ($has_cards) {
                                        // حالة اكتمال جميع المهام
                                        if ($done_cards == $total_cards && $total_cards > 0) {
                                            // إذا اكتملت جميع المهام بعد تاريخ الانتهاء
                                            if ($project['end_date'] < $current_date) {
                                                $project_status = 'Completed (Late)';
                                            } else {
                                                $project_status = 'Completed';
                                            }
                                        }
                                        // حالة انتهاء المدة مع وجود مهام غير مكتملة
                                        elseif ($project['end_date'] < $current_date) {
                                            $project_status = 'Over Due';
                                        }
                                        // حالة وجود مهام قيد التنفيذ
                                        elseif ($in_progress_cards > 0) {
                                            $project_status = 'In Progress';
                                        }
                                        // حالة وجود مهام متأخرة
                                        elseif ($has_overdue_cards) {
                                            $project_status = 'Over Due';
                                        }
                                        // حالة وجود مهام ولكن لم يبدأ العمل عليها بعد
