                ...
                <div class="tasks-table-container">
                    <table class="tasks-table">
                       ...
                       SELECT 
                           ...
                           IFNULL(card_details.status, 'To Do') as status,  -- تأكيد وجود قيمة افتراضية
                       ...
                       WHERE projects.created_by = '$user' OR 
                             projects.members LIKE '%$user%' OR
                             projects.project_managers LIKE '%$user%'
                       GROUP BY cards.id
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                projectFilter.addEventListener('change', function() {
                    ...
                    rows.forEach(row => {
                        ...
                        // Get the project name from the second column (td:nth-child(2))
                        const projectName = row.querySelector('td:nth-child(2)').textContent;

                        // Find the option that matches this project name
                        const option = Array.from(projectFilter.options).find(opt => opt.textContent === projectName);

                        // Show row if the option's value matches the filter value
                        row.style.display = (option && option.value === filterValue) ? '' : 'none';
                    });
                });
                ...
                statusFilter.addEventListener('change', function() {
                    ...
                    rows.forEach(row => {
                        ...
                        // الحصول على حالة المهمة من زر "Read More" بدلاً من الـ badge
